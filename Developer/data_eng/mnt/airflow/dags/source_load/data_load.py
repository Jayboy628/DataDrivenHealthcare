...
from config import SNOWFLAKE_CONFIG, SNOWFLAKE_TABLES, SCHEMA_FILE_MAPPING
...
def load_data_from_stage_to_table(cur, file_name):
    try:
        ...
        schema_name = None
        for schema, files in SCHEMA_FILE_MAPPING.items():
            if table_name.lower() in files:
                schema_name = schema
                break
        
        if not schema_name:
            print(f"Unable to determine schema for {file_name}. Skipping...")
            return
        ...
        print(f"Data loaded into {schema_name}.{table_name} from {file_name}.")
